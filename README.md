...
stateDiagram-v2
    ...
    SUBMITTED --> PROCESSING : StartProcessing
    SUBMITTED --> RETRY_WAITING : RetryWait
    SUBMITTED --> FAILED : Fail

    PROCESSING --> COMPLETED : Complete
    PROCESSING --> RETRY_WAITING : RetryWait
    PROCESSING --> FAILED : Fail

    RETRY_WAITING --> SUBMITTED : RecoverToSubmitted
    RETRY_WAITING --> FAILED : Fail
...
서버(app-consumer) 시작 시 `PollingLifecycleManager`(`SmartLifecycle`)가 미완료 작업을 자동 복구합니다:
